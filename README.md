...
- 달력에서 시작일·종료일 클릭 또는 PC 드래그로 기간 선택, 선택 범위 표시와 예약 불가일 안내
- 가족 전용 자유게시판: 글 목록·작성·조회, 본인 글 수정·삭제, 관리자 삭제, 20개씩 목록 조회
...
달력 선택 로직은 Node.js 22 이상에서 `npm test`로 검증합니다. npm 패키지 설치나 프런트엔드 빌드는 필요하지 않습니다.
게시판 권한·입력 길이·동시 수정과 PostgreSQL 비공개 접근 제한은 아래 통합 테스트에 포함됩니다.
